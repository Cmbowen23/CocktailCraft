...
const CustomLegend = (props) => {
  ...
  return (
    <div className="max-h-[280px] overflow-y-auto text-sm text-gray-700 pl-4">
      <h3 className="font-bold text-base text-gray-800 mb-2">Suppliers</h3>
...
export default function SupplierChart({ allIngredients = [], recipes = [] }) {
  const supplierData = useMemo(() => {
    if (!Array.isArray(recipes) || recipes.length === 0 || !Array.isArray(allIngredients) || allIngredients.length === 0) {
      return [];
    }

    const supplierCounts = recipes.reduce((acc, recipe) => {
      const uniqueSuppliersInRecipe = new Set();
      
      (recipe.ingredients || []).forEach(ing => {
        const matchedIngredient = findMatchingIngredient(ing.ingredient_name, allIngredients);
        if (matchedIngredient && matchedIngredient.supplier && isAlcoholicIngredient(matchedIngredient)) {
          uniqueSuppliersInRecipe.add(matchedIngredient.supplier.trim());
        }
      });

      uniqueSuppliersInRecipe.forEach(supplier => {
    ...
    return Object.entries(supplierCounts).map(([name, value]) => ({
      name: name === 'House' ? 'House Made' : name,
      value,
  ...
  if (supplierData.length === 0) {
    return (
      <div className="flex items-center justify-center h-full min-h-[300px] text-gray-500">
        No alcoholic supplier data available for this menu.
      </div>
    );
  }
