...
const INGREDIENT_FIELDS = [
  { key: 'name', label: 'Name', required: true, aliases: ['name', 'item', 'product'] },
  { key: 'category', label: 'Category', required: true, aliases: ['category', 'type', 'group'] },
  { key: 'spirit_type', label: 'Spirit Type', required: false, aliases: ['spirit type', 'liquor type', 'subtype'] },
  { key: 'substyle', label: 'Substyle', required: false, aliases: ['substyle'] },
  { key: 'flavor', label: 'Flavor', required: false, aliases: ['flavor'] },
  { key: 'region', label: 'Region', required: false, aliases: ['region'] },
  { key: 'supplier', label: 'Supplier', required: false, aliases: ['supplier', 'vendor'] },
  { key: 'sku_number', label: 'SKU Number', required: false, aliases: ['sku', 'id', 'code'] },
  { key: 'exclusive', label: 'Exclusive', required: false, aliases: ['exclusive'] },
  { key: 'tier', label: 'Tier', required: false, aliases: ['tier'] },
  { key: 'purchase_price', label: 'Purchase Price', required: false, aliases: ['price', 'cost', 'unit cost'] },
  { key: 'purchase_quantity', label: 'Purchase Quantity', required: false, aliases: ['size', 'volume', 'qty'] },
  { key: 'purchase_unit', label: 'Purchase Unit', required: false, aliases: ['unit', 'uom'] },
  { key: 'case_price', label: 'Case Price', required: false, aliases: ['case price', 'case cost'] },
  { key: 'bottles_per_case', label: 'Bottles Per Case', required: false, aliases: ['pack', 'bottles', 'case size'] },
  { key: 'abv', label: 'ABV', required: false, aliases: ['abv', 'alcohol', 'proof'] },
  { key: 'description', label: 'Description', required: false, aliases: ['description', 'notes'] },
  { key: 'bottle_image_url', label: 'Image URL', required: false, aliases: ['image', 'url', 'photo'] },
];
...
export default function BulkIngredientSpreadsheetImporter({ onComplete, onCancel }) {
  ...
  const handleParseFile = async () => {
    ...
    try {
      ...
      INGREDIENT_FIELDS.forEach(field => {
        ...
        if (!match) {
            // Check aliases
            match = headers.find(h => field.aliases.some(alias => h.includes(alias)));
        }
        initialMappings[field.key] = match || '__unmapped__'; // FIX: Default to valid Select value
      });
  ...
  const handleConfirmMappings = async () => {
    ...
    setTimeout(() => {
        try {
            // Group rows by Name + Supplier
            const groups = {};
            parsedRows.forEach(row => {
                const mapRow = {};
                Object.entries(columnMappings).forEach(([key, col]) => {
                    if (col && col !== '__unmapped__') mapRow[key] = row[col];
                });
                ...
                if (!groups[key]) {
                    groups[key] = {
                        ...
                        bottle_image_url: mapRow.bottle_image_url,
                        variants: []
                    };
                }
                ...
                groups[key].variants.push({
                    ...
                    purchase_unit: unit,
                    bottle_image_url: mapRow.bottle_image_url // Variant level image override
                });
            });

            // Compare against DB
            const cleanedIngredients = Object.values(groups).map(ing => {
                ...
                const ingChanges = [];
                if (exactMatch) {
                    ['category','tier','spirit_type','supplier', 'bottle_image_url'].forEach(f => {
                        const newVal = ing[f];
                ...
                const variantAnalysis = ing.variants.map(v => {
                    ...
                    if (exactMatch) {
                        ...
                        if (v.sku_number) existingVar = existingVars.find(ev => normalizeName(ev.sku_number) === normalizeName(v.sku_number));
                        if (!existingVar && v.size_ml > 0) existingVar = existingVars.find(ev => Math.abs(ev.size_ml - v.size_ml) < 5);
                    }
                    ...
                    if (existingVar) {
                        ...
                        check('bottles_per_case', v.bottles_per_case, existingVar.bottles_per_case);
                    }
    ...
    for (let i=0; i<parsedIngredients.length; i++) {
        ...
        try {
            ...
            const ingPayload = {
                ...
                exclusive: ingData.exclusive, bottle_image_url: ingData.bottle_image_url
            };
  ...
  if (step === 'mapping') return (
    <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm max-w-4xl mx-auto">
      <CardHeader><CardTitle className="text-emerald-900">Map Columns</CardTitle></CardHeader>
      <CardContent>
            ...
            <div className="grid grid-cols-2 gap-4 max-h-[500px] overflow-y-auto p-2">
                {INGREDIENT_FIELDS.map(field => (
                    <div key={field.key} className="flex flex-col gap-1">
                        <Label className="text-xs font-medium uppercase text-gray-500">{field.label}</Label>
                        {/* FIX: Ensure value is valid (defaults to __unmapped__) */}
                        <Select 
                            value={columnMappings[field.key] || '__unmapped__'} 
                            onValueChange={v => setColumnMappings(prev => ({...prev, [field.key]: v}))}
                        >
  ...
  if (step === 'confirm') return (
    <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm max-w-5xl mx-auto">
      <CardHeader><CardTitle className="text-emerald-900">Review & Import</CardTitle></CardHeader>
      <CardContent>
        <div className="space-y-4">
